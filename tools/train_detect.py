...
class MtcnnDetector(object):
    ...
    def __init__(self,
        ...
        # self.anchors = [[1,0.5],[1,1],[1,2],[1,3]] #[h,w]
        self.anchors = [[1,1],[1,3]] #[h,w]
    ...
    def generate_bounding_box(self, map, reg, scale, threshold, anchor):
        ...
        boundingbox = np.vstack([np.round((stride * t_index[1]) / scale),
                                 ...
                                 np.round(
                                     (stride * t_index[1] + cellsize*anchor[1]) / scale),
                                 np.round(
                                     (stride * t_index[0] + cellsize*anchor[0]) / scale),
                                 score,
                                 reg,
                                 # landmarks
                                 ])
    ...
    def detect_pnet(self, im):
        ...
        # for i in range(len(align_bottomx)):
        #     cv2.rectangle(im,(int(align_topx[i]),int(align_topy[i])),(int(align_bottomx[i]),int(align_bottomy[i])),(0,0,255))
        # cv2.imshow('im',im)
        # cv2.waitKey()
    ...
    def detect_onet(self, im, dets):
        ...
        cls_map, reg, landmark, attr = self.onet_detector(feed_imgs)
        ...
        keep = utils.nms(boxes, 0.4, mode="Minimum")
        ...
        align_color = torch.max(keep_attr[:,:5],1)
        align_layer = torch.max(keep_attr[:,5:7],1)
        align_type  = torch.max(keep_attr[:,7:],1)
        align_attr  = np.vstack([align_color[1],
                                 align_layer[1],
                                 align_type[1]])
        attr_align = align_attr.T
        
        return boxes_align, landmark_align, attr_align
    ...
    def detect_face(self, img):
        ...
        if self.rnet_detector:
            ...
            if boxes_align is None:
                return np.array([]), np.array([]), np.array([]), pboxes, np.array([])
        ...
        if self.onet_detector:
            boxes_align, landmark_align, attr_align = self.detect_onet(img, boxes_align)
            if boxes_align is None:
                return np.array([]), np.array([]), np.array([]), pboxes, rboxes
        ...
        return boxes_align, landmark_align, attr_align, pboxes, rboxes
