...
class ImageDB(object):
    ...
    def load_annotations(self, annotation_type=1):
        ...
        for i in range(self.num_images):
            ...
            if self.mode == 'test':
                ...
                imdb_['attribute'] = np.zeros((3,))
                if len(annotation[2:]) == 4:
                    bbox_target = annotation[2:6]
                    imdb_['bbox_target'] = np.array(bbox_target).astype(float)
                if len(annotation[2:]) == 12:
                    bbox_target = annotation[2:6]
                    imdb_['bbox_target'] = np.array(bbox_target).astype(float)
                    landmark = annotation[6:]
                    imdb_['landmark_target'] = np.array(landmark).astype(float)
                if len(annotation[2:]) == 15:
                    bbox_target = annotation[2:6]
                    imdb_['bbox_target'] = np.array(bbox_target).astype(float)
                    landmark = annotation[6:14]
                    imdb_['landmark_target'] = np.array(landmark).astype(float)
                    attr = [annotation[14],annotation[15],annotation[16]]
                    imdb_['attribute'] = np.array(attr).astype(float)
            imdb.append(imdb_)
        return imdb
    ...
    def append_flipped_images(self, imdb):
        ...
        for i in range(len(imdb)):
            ...
            landmark_[[2, 3]] = landmark_[[3, 2]]

            item = {'image': imdb_['image'],
                    'label': imdb_['label'],
                    'attribute': imdb_['attribute'],
                    'bbox_target': m_bbox,
...
class FaceDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels['attribute'] = imdb_['attribute']
