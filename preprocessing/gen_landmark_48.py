...
def gen_data(anno_file, data_dir, prefix):
    ...
    for annotation in annotations:
        ...
        if len(annotation) != 16:
            print("each line should have 16 element")
            print(annotation)
            continue
        ...
        landmark = list(map(float, annotation[5:13]))
        landmark = np.array(landmark, dtype=np.float)

        attr = list(map(int, annotation[13:]))
        attr = np.array(attr, dtype=np.int32)
        ...
        for i in range(20):
            ...
            bbox_size_w = npr.randint(int(w * 0.8), np.ceil(1.25 * w))
            bbox_size_h = npr.randint(int(h * 0.8), np.ceil(1.25 * h))
            delta_x = npr.randint(-w * 0.2, w * 0.2)
            delta_y = npr.randint(-h * 0.2, h * 0.2)
            nx1 = int(max(x1 + w / 2 - bbox_size_w / 2 + delta_x, 0))
            ny1 = int(max(y1 + h / 2 - bbox_size_h / 2 + delta_y, 0))
            ...
            offset_x1 = (x1 - nx1) / float(bbox_size_w)
            offset_y1 = (y1 - ny1) / float(bbox_size_h)
            offset_x2 = (x2 - nx2) / float(bbox_size_w)
            offset_y2 = (y2 - ny2) / float(bbox_size_h)

            offset_left_eye_x = (landmark[0] - nx1) / float(bbox_size_w)
            offset_left_eye_y = (landmark[1] - ny1) / float(bbox_size_h)

            offset_right_eye_x = (landmark[2] - nx1) / float(bbox_size_w)
            offset_right_eye_y = (landmark[3] - ny1) / float(bbox_size_h)

            offset_nose_x = (landmark[4] - nx1) / float(bbox_size_w)
            offset_nose_y = (landmark[5] - ny1) / float(bbox_size_h)

            offset_left_mouth_x = (landmark[6] - nx1) / float(bbox_size_w)
            offset_left_mouth_y = (landmark[7] - ny1) / float(bbox_size_h)

            # offset_right_mouth_x = (landmark[8] - nx1) / float(bbox_size_w)
            # offset_right_mouth_y = (landmark[9] - ny1) / float(bbox_size_h)
            ...
            if iou > 0.65:
                ...
                f.write(save_file + ' -2 %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %d %d %d\n' %
                        (offset_x1, offset_y1, offset_x2, offset_y2,
                         offset_left_eye_x, offset_left_eye_y, offset_right_eye_x, offset_right_eye_y, offset_nose_x,
                         offset_nose_y, offset_left_mouth_x, offset_left_mouth_y, 
...
def parse_args():
    ...
    parser.add_argument('--anno_file', dest='annotation_file', help='landmark dataset original annotation file',
                        default=os.path.join(config.ANNO_STORE_DIR, "landmark_list.txt"), type=str)
